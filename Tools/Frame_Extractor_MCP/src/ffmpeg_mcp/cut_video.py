...
def enhance_frames(input_folder, output_folder=None, model="realesrgan-x4plus-anime", scale=4, format=0, in_place=False):
    """
    使用 Real-ESRGAN (ncnn-vulkan) 对图像帧进行超分辨率增强（提升画质/分辨率）。
    ...
    model(str) - 使用的模型名称，可选：
        realesrgan-x4plus-anime (动漫专用，推荐)
        realesrgan-x4plus (通用模型)
        realesr-animevideov3 (动漫视频专用，速度更快)
    scale(int) - 放大倍数，2/3/4，默认4
    format(int) - 输出图片格式 0:png 1:jpg 2:webp
    in_place(bool) - 是否将增强后的图片直接替换 input_folder 中的原始帧
        （忽略 output_folder，处理完成后原始帧会被覆盖）
    """
    tools_dir = _find_tools_dir()
    if tools_dir is None:
        return -1, "未找到 Tools 目录，无法定位 RealESRGAN", ""

    exe_path = os.path.join(tools_dir, "RealESRGAN", "realesrgan-ncnn-vulkan.exe")
...
def extract_frames_from_video(video_path,fps=0, output_folder=None, format=0, total_frames=0):
    ...
    if output_folder == None:
        ...
        # Tools/Frame_Extractor_MCP/src/ffmpeg_mcp/).
