...
def resolve_binary(name):
    """Return the full path to an ffmpeg-family binary.

    On macOS the project ships/downloads its own static build into bin/.
    On every other platform (e.g. Windows, Linux) fall back to the binary
    found on PATH, so a system-installed ffmpeg is used.
    """
    system, _ = check_os_architecture()
    if system == "Darwin":
        cmd_dir = command_dir()
        if cmd_dir is None:
            return None
        return f"{cmd_dir}/{name}"
